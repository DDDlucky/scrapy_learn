...
class DesktopSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #     maker = response.xpath('.//div[contains(@class, "pName")]/p/text()').get()
        #     # maker = response.css('div.pName > p ::text')
        #     name = response.xpath('.//div[contains(@class, "pName")]/p[2]/text()').get()
        #     price = response.xpath('.//span[@class="productPrice"]/text()').get()
        #     yield {
        #         'maker': maker,
        #         'name': name,
        #         'price': price
